...
<template>
  <div class="page-container">
    ...
    <div class="toolbar" style="float:left;padding-top:10px;padding-left:15px;">
      <el-form :inline="true" :model="filters" :size="size" ref="filters" class="form">
        <el-form-item prop="card_num">
          <el-input v-model="filters.title" auto-complete="off" placeholder="酒店标题"></el-input>
        </el-form-item>
    ...
    <el-dialog :title="operation?'新增':'编辑'" :visible.sync="checkpage"  width="60%"  @closed="handleClose">
      ...
      >
      <el-row>
          <el-form-item label="酒店标题" prop="title">
            <el-input v-model="dataForm.title" auto-complete="off"></el-input>
          </el-form-item>
            <el-form-item label="酒店分类" prop="type">
            <el-select v-model="dataForm.type" style="width:200px">
              <el-option
          ...
          <el-form-item label="酒店评分" prop="fraction">
            <el-input v-model="dataForm.fraction" auto-complete="off"></el-input>
          </el-form-item>
          <el-form-item label="是否精选" prop="is_selected" >
       ...
       <!--轮播图 -->
          <el-form-item label="轮播图" prop="carousel">
            <div style="float: left;">
              <ul class="el-upload-list el-upload-list--picture-card">
                <li
                  v-for="(bill_photo,index) in bill_photo_list2"
                  :key="index"
                  :tabindex="index"
                  class="el-upload-list__item is-success"
                >
                  ...
                  <span class="el-upload-list__item-actions">
                    ...
                    <span class="el-upload-list__item-delete">
                      <i class="el-icon-delete" @click="handleRemove2(null,null,index)"></i>
                    </span>
                  </span>
                </li>
              </ul>
    ...
    <el-dialog :visible.sync="dialogVisibleImageList" title="图片展示" >
      <div>
      ...
      this.$refs.dataForm.validate(valid => {
        if (valid) {
          this.$confirm("确认提交吗？", "提示", {}).then(() => {
            ...
            documentlicense2 = documentlicense2.concat(this.bill_photo_list2);
            ...
            params.pic = documentlicense
              ...
              .join(",");
              //过滤多余的','
            params.carousel = documentlicense2
              .filter(function(e) {
                if (this_.utils.isNull(e)) {
                } else {
                  return e;
   ...
   handleRemove2(file, fileList, index) {
      if (this.utils.isNull(index)) {
        ...
        if (fileList.length == 0) {
          ...
          this.imgs2 = fileList
            .map(item => item.response.data.imgUrl)
            .toString();
          this.dataForm.carousel = this.imgs2;
        }
      } else {
        this.bill_photo_list.splice(index, 1);
      }
      console.log(this.dataForm.carousel);
    },
    ...
    handleAvatarSuccess2(res, file) {
      ...
      if (res.data.imgUrl == "") {
        ...
        this_.dataForm.carousel += res.data.imgUrl + ",";
        console.log(this_.dataForm.carousel);
        this_.imageUrl = URL.createObjectURL(file.raw);
      }
    },
    //列表下载
    ...
    initColumns: function() {
      this.columns = [
        { prop: "title", label: "酒店标题", minWidth: 120 },
        { prop: "type", label: "分类", minWidth: 120 ,formatter: this.showtype},
        { prop: "fraction", label: "评分", minWidth: 120 },
        { prop: "pic", label: "主图图片", minWidth: 180 ,formatter:this.showPhto},
        { prop: "carousel", label: "轮播图图片", minWidth: 180 ,formatter:this.showPhto},
        { prop: "is_selected",label: "是否精选", minWidth: 120,formatter: this.showselect},
